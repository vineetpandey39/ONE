...
class TestAudioTranscribeTool:
    ...
    def test_local_provider_success(self, tmp_path):
        f = tmp_path / "audio.wav"
        f.write_bytes(b"\x00" * 100)
        transcription = MagicMock(
            text="ONE, activate Athena.",
            language="en",
            duration_seconds=1.25,
        )
        backend = MagicMock(backend_id="faster-whisper")
        backend.transcribe.return_value = transcription
        AudioTranscribeTool._local_backend = None
        ...
        assert result.content == "ONE, activate Athena."
