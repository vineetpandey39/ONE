...
def create_agent_manager_router(
    ...
    # ── Custom (tool-independent) credentials ───────────────
    # Distinct path segment from "/{tool_name}/credentials" above so there's
    # no route-ordering ambiguity — this lets users add any env var from the
    # wallet UI without it needing a predefined TOOL_CREDENTIALS entry.

    @tools_router.get("/custom-credentials")
    def list_custom_creds():
        from openjarvis.core.credentials import list_custom_credentials

        return list_custom_credentials()

    @tools_router.post("/custom-credentials")
    async def add_custom_cred(request: Request):
        from openjarvis.core.credentials import save_custom_credential
        ...
        try:
            save_custom_credential(key, value)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc))
        return {"saved": key}

    @tools_router.delete("/custom-credentials/{key}")
    def remove_custom_cred(key: str):
        from openjarvis.core.credentials import delete_custom_credential

        delete_custom_credential(key)
        return {"deleted": key}
