...
def create_app(
    ...
    *,
    ...
    """
    # Load any credentials saved via the Tools credential picker (or written
    # directly to credentials.toml) into this process's os.environ. The
    # Tauri desktop app launches the server via this factory directly (not
    # always through `jarvis serve`'s CLI group), so this needs to happen
    # here too — idempotent, never overwrites an already-set env var.
    from openjarvis.core.credentials import inject_credentials

    inject_credentials()

    # Start the one_agents job-queue worker (ALFA/TITAN/BETA/etc.).
    # enqueue_job() only inserts a 'queued' row in agent_queue.db — nothing
    # else in the process ever consumed that queue, so every queued job sat
    # forever and never actually ran. run_worker() polls and executes jobs;
