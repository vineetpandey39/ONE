...
class ToolExecutor:
    ...
    def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        # Parse arguments.
        #
        # Small local models (e.g. qwen3.5:2b driving the text-based ReAct
        # loop) frequently fail to emit strict JSON for "Action Input:" —
        # Python-dict quoting, trailing prose, or just the bare prompt text
        # with no braces at all. A hard `json.JSONDecodeError` there silently
        # drops the user's actual request (e.g. `prompt` for image_generate)
        # and the tool fails with a generic "no prompt provided" that the
        # model then has to rationalize back to the user. `_parse_arguments`
        # tries JSON first, then a Python-literal fallback, then — only when
        # the tool declares exactly one required parameter — wraps the raw
        # text as that parameter so the original intent isn't lost.
        params, parse_error = self._parse_arguments(tool_call.arguments, tool)
    ...
    def _parse_arguments(
        raw: str, tool: "BaseTool"
    ) -> "tuple[Dict[str, Any], Optional[str]]":
        """Tolerantly parse a tool call's argument string.

        Returns ``(params, None)`` on success or ``({}, error_message)`` on
        failure. Tries, in order: strict JSON, Python-literal syntax (single
        quotes, trailing commas — common from smaller local models), then —
        only if the tool's schema declares exactly one required parameter —
        treats the entire raw string as that parameter's value so a bare,
        unquoted prompt isn't dropped on the floor.
        """
        if not raw:
            return {}, None
        ...
        if len(required) == 1:
            return {required[0]: raw.strip().strip("\"'")}, None

        return {}, f"Invalid arguments JSON: could not parse '{raw}'"
