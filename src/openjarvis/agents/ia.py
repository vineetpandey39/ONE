"""IA (Imagine India) universal restoration-reel engine.
...
from openjarvis.agents.ia_dashboard import DashboardRun
from openjarvis.agents.ia_prompts import (
    build_clip_prompts,
    build_dialogue_line,
    build_frame_prompts,
    build_hook_lines,
...
class IAAgent(ToolUsingAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        # voice, VEED Fabric 1.0 for lip-sync). DEFAULT OFF as of the Bible
        # v2.0 rollout: this 3-step sub-pipeline (image/TTS/lip-sync) kept
        # failing in practice, and the reel's payoff moment is now the
        # dedicated 8-second Hero Reveal finale (clip E) instead -- the
        # silent locked-aerial A-E sequence is the whole reel by default.
        # The method/plumbing is left fully intact (not deleted) so it can
        # be switched back on with enable_intro_clip=True if the lip-sync
        # backend is ever made reliable.
        self._enable_intro_clip = kwargs.pop("enable_intro_clip", False)
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        for clip in clips:
            ...
            # Per-clip target duration from the Bible/Templates spec (A-D
            # 4-5s, Hero Reveal E 8s) -- single source of truth is
            # ia_prompts._CLIP_DURATION_SECONDS, exposed on each clip dict as
            # "duration_seconds" by build_clip_prompts(). Defaults to 5 if
            # somehow missing so older clip dicts without the field still work.
            clip_duration = clip.get("duration_seconds", 5)
            if use_browser:
                ...
                args: Dict[str, Any] = {
                    ...
                    # any integer duration unvalidated -- this is what makes
                    # the 8s Hero Reveal vs 5s A-D actually real, not just
                    # text in the prompt.
                ...
                if start_frame == end_frame:
                    ...
                    # video_generate's single-image (image-to-video) mode --
                    # the path clip E (Hero Reveal) takes on this backend --
                    # only accepts duration "5" or "10" (validated strictly in
                    # video_tool.py); 8 isn't a legal value, so round up to
                    # "10" rather than truncating the finale to 5s. A-D never
                    # hit this branch (they have distinct start/end frames),
                    # so their 5s spec is untouched.
                    fal_duration = "10" if clip_duration >= 8 else "5"
                    args = {
                    ...
                    # restoration-progression clips (A-D). The flf2v model has
                    # no external duration knob (video_tool.py never forwards
                    # "duration" on this branch), so the 4-5s A-D spec is
                    # already satisfied by the model's fixed clip length --
